...
import { intifaceService, IntifaceDevice } from '../services/intiface';
import * as hapticApi from '../api/haptic';

export interface LocalDevice extends IntifaceDevice {
  battery?: number;
  synced: boolean; // Whether this device is synced to the backend
  backendId?: string; // Backend device ID if synced
}
...
export function useHaptic() {
  ...
  useEffect(() => {
    ...
    setIsConnected(intifaceService.connected);
    if (intifaceService.connected) {
